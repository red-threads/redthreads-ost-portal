...
Version `1010` adds Standard ACH bank-verification 4-calendar-day reminder coverage. The scheduler is layered onto the existing daily handler `processPurchaseOrderPaymentReminderSchedule`, so the installed trigger remains compatible while running both PO reminder checks and stale ACH verification checks. New sendable review examples are `Standard ACH verification 4-day reminder client` and `Standard ACH verification 4-day reminder team`, both using `payment_verification_required` lifecycle intent. Per owner instruction, no protected dry run was run. The final live suite on version `1010` sent `2` actual emails, skipped `61`, failed `0`, reported `2` attachment fallbacks, and reported `0` contradiction warnings/errors; matrix validation against the live output returned `ok:true` with `37` required cases, `1` covered, `36` skipped/omitted, `0` missing, and `0` intent mismatches. A transient version `1009` sent the same two reminder examples before it was superseded by `1010` to correct review CTA metadata from `pay` to `view_project`. The final `1010` client reminder reports `ctaMode: view_project`.

Version `1012` completes the portal email subject-review tranche. Owner-review omission maps are intentionally cleared again so all configured review emails are visible unless a true fixture/artifact safety skip applies. Production communication eligibility, suppression, recipient routing, attachment policy, and lifecycle contract rules are unchanged. Production subject builders now use short audience-aware subject helpers, and old/new subject examples are tracked in `docs/EMAIL_SUBJECT_AUDIT.md`. The protected post-deploy dry run returned `ok:true`, total `76`, skipped `1`, failed `0`, attachment fallback `44`, contradiction warnings/errors `0`, and strict matrix `ok:true` with `37 / 37` required cases covered and `0` intent mismatches. The owner-approved live suite sent `62` review emails (`34` client, `24` team, `4` AP), skipped only `Submitted tax-form copy client`, failed `0`, reported `44` attachment fallbacks, and reported `0` contradiction warnings/errors. The live non-dry-run path reset active runtime tabs from fixture storage and cleared `PORTAL_EMAIL_QUEUE`; fixture-storage tabs were not mutated. The live send artifact is an inbox-send receipt and does not include dry-run-only assertion rows, so protected dry-run JSON remains the authoritative full matrix validation artifact.
...
| --- | --- | --- | --- | --- |
| Standard ACH client emails | `Standard ACH pending client`, `Standard ACH verification client`, `Standard ACH verification 4-day reminder client`, `Standard ACH receipt client`, `Standard ACH failed client` | Order-context progress + Project Details | Show `Production Time` when selected-job turnaround is available; progress shows `Will Finish M/D` when production is current | Pending/verification/receipt/failed labels remain hidden after owner review; the 4-day verification reminder client example is sendable and was sent in version `1010` |
| Standard ACH team emails | `Standard ACH pending team`, `Standard ACH verification team`, `Standard ACH verification 4-day reminder team`, `Standard ACH receipt team`, `Standard ACH failed team` | Order-context progress + Project Details + client identity fields | Show `Production Time` when selected-job turnaround is available; progress matches the client lifecycle display while next-action copy remains team-specific | Pending, verification, and receipt team labels hidden in version `1002`; failed team hidden in version `1008`; the 4-day verification reminder team example is sendable and was sent in version `1010` |
...
| `ach_verification_4_day_reminder` | Client + team | Sendable scheduled reminder coverage added in version `1010` |
...
Version `1015` restores the active runtime tabs from the private pre-fixture Drive backup after explicit owner approval. The restore mutated only `EXPORT_LOG`, `PORTAL_ORDERS`, and `PORTAL_STRIPE_EVENTS` below preserved headers, created a private rollback workbook copy, left `FIXTURE_*` tabs untouched, left `PORTAL_EMAIL_QUEUE` unchanged at two `sent` rows, and kept the scheduler installed once. Post-restore `npm run email-review:status` reports `activeTabState: live_like`, with `EXPORT_LOG`, `PORTAL_ORDERS`, and `PORTAL_STRIPE_EVENTS` all at fixture match mode `none`. Bounded sentinel searches found `0` active `Fixture`/`EmailReview` matches across the restored tabs. The no-mutation storage-source dry run remains clean with `ok:true`, total `91`, sent `0`, skipped `1`, failed `0`, attachment fallback `44`, contradiction warnings/errors `0`, parity failures `0`, `activeTabsMutated:false`, `queueCleared:false`, and matrix `ok:true` with `37 / 37` required cases covered and `0` intent mismatches.

The first safe normalization step is complete: duplicated fixture-storage blocks were cleared while preserving headers and the existing canonical fixture rows. The second buildout added dedicated lifecycle rows for the missing safe states. The first controlled active reset for dry-run validation is also complete. The first live owner review-suite blast against the active fixture matrix is complete, the unlocked full review-suite blasts against version `985` are complete, versions `986`/`987` temporarily hid accepted/pass communications from future suite sends, version `989` re-unlocked the suite and completed another full live blast, version `990` re-hides the newly owner-reviewed/pass labels without running the email suite, version `991` adds sendable production-complete pickup/shipping coverage without a live suite send, version `992` refines team action-card status labels while hiding additional owner-reviewed team labels and running the live suite, version `993` adds sendable PO reminder/late-fee schedule coverage and completed an owner-approved live suite, version `997` installs the daily PO reminder trigger with owner-approved fixture-mode activation, version `998` refines client action/timing copy and completes another owner-approved live suite, version `999` refines production-complete pickup plus PO reminder copy and completes another owner-approved live suite, version `1010` adds sendable ACH verification 4-calendar-day reminder client/team coverage and completes the owner-approved live suite, and version `1015` restores active runtime tabs to live-like data. Future fixture review should use `--fixture-source=storage` unless an intentional active-tab fixture reset is approved.
...
4. The daily scheduled-email trigger is installed and now runs both PO payment reminder checks and ACH verification reminder checks against restored live-like runtime data.
5. Use storage-source fixture review mode for future inbox/test review unless the owner explicitly approves another active runtime fixture reset.
