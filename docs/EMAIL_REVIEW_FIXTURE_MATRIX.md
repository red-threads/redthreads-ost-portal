...
4. The daily scheduled-email trigger is installed under explicit owner-approved fixture-mode activation and now runs both PO payment reminder checks and ACH verification reminder checks; restore active runtime tabs when ready to leave fixture mode.
5. Have the owner inspect the delivered ACH verification 4-day reminder client/team emails, then keep active tabs in fixture mode or restore them from the private Drive backup only after explicit owner direction.
