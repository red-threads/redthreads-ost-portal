...
- Remaining live-smoke evidence still required before claiming every matrix path complete: AP ACH failed/action-needed, manual payment received, PO payment received, and Team Mode/admin lifecycle transitions. AP ACH failure requires either a human-completed Stripe Checkout challenge, a supported Stripe test-event path, or another owner-approved Stripe test path. Manual/PO received and Team Mode/admin transitions require a corrected live Team Mode credential or an already-authorized Team Mode browser session.
