...
- Follow-ups: manually re-test the AP submit-payment path through Stripe test mode and confirm Stripe Checkout asks for a contact email instead of locking `josiah@redthreads.com`. Also re-check a normal owner ACH payment path to confirm the same editable Stripe email behavior, while Dashboard Add ACH bank still uses hosted setup for account bank readiness.
